...
if ($LOGGING) {
    ...
    $LOGFILEPREFIX = $1
      ; # for some reason, $0 is considered tainted here, but not in other cases...
    mkdir( $LOGFILEDIR, 0700 )
      ;    # if this fails for a bad reason, we'll find out during the next line
}
...
if ( $ENV{'PATH'} =~ /^(?:.*:)?\.?(?::.*)?$/os ) {
    die
'SECURITY RISK. You cannot have \'.\' in the path. See the README. Aborted';
}
$ENV{'PATH'} =~ /^(.*)$/os;
$ENV{'PATH'} =
  $1;    # we have to assume their path is otherwise safe, they called us!
...
my (
    $server,       $port,     $password,    $localAddr,
    @nicks,        @channels, %channelKeys, $owner,
...
my $recentMessageCountPenalty =
  10;    # if we hit the threshold, bump it up by this much
...
my %users =
  ( 'admin' => &newPassword('password') );    # default password for admin
my %userFlags = ( 'admin' => 3 )
  ; # bitmask; 0x1 = admin, 0x2 = delete user a soon as other admin authenticates
my $helpline =
  'http://www.mozilla.org/projects/mozbot/';    # used in IRC name and in help
...
# It will abort otherwise.
{
    ...
    $changed = &Configuration::Ensure(
        [
            ...
            [
'What is the server\'s password? (Leave blank if there isn\'t one.)',
                \$password
            ],
    ...
    until (@nicks) {
        $changed = &Configuration::Ensure(
            [ [ 'What nicks should I use? (I need at least one.)', \@nicks ] ] )
          || $changed;
...
my $uptime;
my $irc = new Net::IRC
  or confess("Could not create a new Net::IRC object. Aborting");
...
sub connect {
    ...
    until (
        inet_aton($server)
          and    # we check this first because Net::IRC::Connection doesn't
          $bot = $irc->newconn(
            Server   => $server,
            Port     => $port,
            Password => $password ne ''
            ? $password
            : undef,    # '' will cause PASS to be sent
            Nick      => $nicks[$nick],
            Ircname   => $ircname,
            Username  => $identd,
            LocalAddr => $localAddr,
    ...
    {
        &debug(
            "Could not connect. Are you sure '$server:$port' is a valid host?");
        unless ( inet_aton($server) ) {
            &debug('I couldn\'t resolve it.');
        }
        if ( defined($localAddr) ) {
            &debug(
                "Is '$localAddr' the correct address of the interface to use?");
        }
        else {
            &debug(
"Try editing '$cfgfile' to set 'localAddr' to the address of the interface to use."
            );
        }
        if ( $Net::IRC::VERSION < 0.73 ) {
            &debug(
"Note that to use 'localAddr' you need Net::IRC version 0.73 or higher (you have $Net::IRC::VERSION)"
            );
        }
        $mailed =
          &Mails::ServerDown( $server, $port, $localAddr, $nicks[$nick],
            $ircname, $identd )
          unless $mailed;
        sleep($sleepdelay);
        &Configuration::Get(
            $cfgfile,
            &configStructure(
                \$server, \$port,  \$password, \@nicks,
                \$nick,   \$owner, \$sleepdelay
            )
        );
    ...
    $bot->add_global_handler(
        [    # Informational messages -- print these to the console
            251,    # RPL_LUSERCLIENT
            252,    # RPL_LUSEROP
    ...
    $bot->add_global_handler(
        [    # should only be one command here - when to join channels
            376,    # RPL_ENDOFMOTD
            422,    # nomotd
        ],
        \&on_connect
    ...
    $bot->add_handler( 'welcome', \&on_set_nick )
      ;             # when we connect, to get our nick
    $bot->add_global_handler(
        [           # when to change nick name
            'erroneusnickname',
    ...
    $bot->add_handler( 'notopic', \&on_topic )
      ;    # when topic in a channel is cleared
    ...
    $bot->add_handler( 'umode',  \&on_umode )
      ;    # when modes of user change (by IRCop or ourselves)
           # XXX could add handler for 474, # ERR_BANNEDFROMCHAN

    $bot->add_handler(
        [    # ones we handle to get our hostmask
            311,    # whoisuser
        ],
        \&on_whois
    );
    $bot->add_handler(
        [           # ones we handle just by outputting to the console
            312,    # whoisserver
            313,    # whoisoperator
            314,    # whowasuser
    ...
    $bot->add_handler(
        [           # names (currently just ignored)
            353,   # RPL_NAMREPLY "<channel> :[[@|+]<nick> [[@|+]<nick> [...]]]"
        ],
        \&on_notice
    );
    $bot->add_handler(
        [ # end of names (we use this to establish that we have entered a channel)
    ...
    $bot->add_handler( 'crping', \&on_cpong )
      ;    # client to client ping (response)
...
sub on_set_nick {
    ...
    ($lastNick) =
      $event
      ->args;    # (args can be either array or scalar, we want the first value)
                 # Find nick's index.
    my $newnick = 0;
    $newnick++
      while ( ( $newnick < @nicks ) and ( $lastNick ne $nicks[$newnick] ) );
...
sub on_nick_taken {
    ...
    if ( $event->type eq 'erroneusnickname' ) {
        my ( $currentNick, $triedNick, $err ) =
          $event->args;    # current, tried, errmsg
    ...
    if ($nickSlept) {
        &debug(
            "waited for a bit -- reading $cfgfile then searching for a nick..."
        );
        ...
        if ( $nick == $nickFirstTried ) {
            ...
            if ( not -t ) {
                ...
                $nickProblemEscalated
                  ||=    # only e-mail once (returns 0 on failure)
                  Mails::NickShortage( $cfgfile, $self->server, $self->port,
                    $self->username, $self->ircname, @nicks ) & debug(
"going to wait $sleepdelay seconds so as not to overload ourselves."
                    );
                $self->schedule( $sleepdelay, \&on_nick_taken, $event, 1 )
                  ;        # try again
            ...
            @nicks = (
                @nicks[ 0 .. $nickFirstTried ],
                $new, @nicks[ $nickFirstTried + 1 .. $#nicks ]
            );
...
sub on_connect {
    ...
    if ( defined( $self->{'__mozbot__shutdown'} ) ) {    # HACK HACK HACK
        &debug(
            'Uh oh. I connected anyway, even though I thought I had timed out.'
        );
...
sub on_check_connect {
    my $self = shift;
    return
      if ( defined( $self->{'__mozbot__shutdown'} )
        or defined( $self->{'__mozbot__active'} ) );    # HACK HACK HACK
...
sub on_disconnected {
    ...
    if (
        $reason =~ /Connection timed out/osi
      ...
      )
    {
        ...
        &Configuration::Save( $cfgfile,
            &configStructure( \$serverRestrictsIRCNames ) );
    ...
    elsif ( $reason =~ /Bad user info/osi
        and $serverRestrictsIRCNames ne $server )
    {
    ...
    elsif ( $reason =~ /identd/osi and $serverExpectsValidUsername ne $server )
    {
        ...
        &Configuration::Save( $cfgfile, &configStructure( \$delaytime ) );
        &debug(
'Hrm, we it seems flooded the server. Trying again with a delay 20% longer.'
        );
        &debug("The full message from the server was: '$reason'");
    }
...
sub targetted {
    ...
    my $ret = undef;
    my $cmdChar =
      $commandChar{default};  # FIXME: should look up for CURRENT channel first!
    if ( exists $commandChar{$chan} ) { $cmdChar = $commandChar{$chan}; }

    if ( $data =~
        /^(\s*$nick(?:[\s,:;.!?]+|\s*:-\s*|\s*--+\s*|\s*-+>?\s+))(.+)$/is )
    {
        $ret = ( defined $2 ? $2 : '' );
    }
    else {
...
sub on_public {
    ...
    if (
        defined(
            $_ = targetted( $data, quotemeta( $nicks[$nick] ), $channel )
        )
      )
    {
        if ( $_ ne '' ) {
        ...
        foreach my $nick (@ignoredTargets) {
            if ( defined targetted( $data, $nick, $channel ) ) {
                ...
                &debug( "Ignored (target matched /$nick/): $channel <"
                      . $event->nick . '> '
                      . join( ' ', $event->args ) );
                return;
            }
...
sub on_cpong {
    my ( $self, $event ) = @_;
    &debug( 'completed CTCP PING with '
          . $event->nick . ': '
          . days( $event->args->[0] ) );
...
sub on_mode {
    &do( @_, 'ModeChange' );
}    # XXX need to parse modes # XXX on key change, change %channelKeys hash
...
sub do {
    ...
    my $e       = newEvent(
        {
            ...
            'fulldata' => defined( $event->{'__mozbot__fulldata'} )
    ...
    if ( $authenticatedUsers{ $event->userhost } ) {
        if (
            ( $userFlags{ $authenticatedUsers{ $event->userhost } } & 1 ) == 1 )
        {
            $lastadmin = $event->nick;
        }
        $e->{'userName'} = $authenticatedUsers{ $event->userhost };
        $e->{'userFlags'} =
          $userFlags{ $authenticatedUsers{ $event->userhost } };
    }
    else {
        $e->{'userName'} = 0;
    }
            ...
            do {
                ...
                foreach my $module (@modulesInThisLoop) {
                    ...
                    if ($@) {
                        ...
                        &debug(
"ERROR IN MODULE $module->{'_name'}: invalid response code to event '$type'."
                        );
                    }
                    else {
...
my @msgqueue;
my %recentMessages;
my $timeLastSetAway = 0
  ; # the time since the away flag was last set, so that we don't set it repeatedly.
...
sub sendmsg {
    ...
    unless (
        ( defined($do) and defined($msg) and defined($who) and ( $who ne '' ) )
        and (
               ( ( $do eq 'msg' )    and ( not ref($msg) ) )
            or ( ( $do eq 'me' )     and ( not ref($msg) ) )
            or ( ( $do eq 'notice' ) and ( not ref($msg) ) )
            or (    ( $do eq 'ctcpSend' )
                and ( ref($msg) eq 'ARRAY' )
                and ( @$msg >= 2 ) )
            or ( ( $do eq 'ctcpReply' ) and ( not ref($msg) ) )
        )
      )
    {
...
sub drainmsgqueue {
    ...
    if ( @msgqueue > 0 ) {
        ...
        unless (
            weHaveSaidThisTooManyTimesAlready( $self, \$who, \$msg, \$do ) )
        {
            my $type;
            if ( $do eq 'msg' ) {
                &debug("->$who: $msg")
                  ;    # XXX this makes logfiles large quickly...
                $self->privmsg( $who, $msg )
                  ;    # it seems 'who' can be an arrayref and it works
                $type = 'Heard';
            }
            elsif ( $do eq 'me' ) {
                &debug("->$who * $msg");    # XXX
                ...
                &debug(
"Unknown action '$do' intended for '$who' (content: '$msg') ignored."
                );
            }
            if ( defined($type) ) {
                &doLog(
        ...
        if ( @msgqueue > 0 ) {
            if (    ( @msgqueue % 10 == 0 )
                and ( time() - $timeLastSetAway > 5 * $delaytime ) )
            {
                &bot_longprocess( $self,
"Long send queue. There were $qln, and I just sent one to $who."
                );
                $timeLastSetAway = time();
                $self->schedule(
                    $delaytime * 4
                    , # because previous one counts as message, plus you want to delay an extra bit regularly
                    \&drainmsgqueue
                );
            }
            else {
...
sub weHaveSaidThisTooManyTimesAlready {
    ...
    if (    $count >= $recentMessageCountThreshold
        and $count < $recentMessageCountThreshold + 1
        and $$do ne 'ctcpSend' )
    {
        ...
        $$do = 'me';
        $$msg =
"was going to say '$text' but has said it too many times today already";
    }
        ...
        if ( $$do eq 'msg' ) {
            ...
            &debug(
"MUTED: Unknown action '$$do' intended for '$$who' (content: '$$msg') ignored."
            );
        }
        return 1;
    }
...
sub yank {
    ...
    @{$list} =
      ( @{$list}[ 0 .. $index - 1 ], @{$list}[ $index + 1 .. $#{$list} ] );
    return $result;
}
...
sub bot_select {
    ...
    my $data = <$pipe>;
    &debug(
"child ${$pipe}->{'BotModules_PID'} completed ${$pipe}->{'BotModules_ChildType'}"
          . (
    ...
    eval {
        ${$pipe}->{'BotModules_Event'}->{'time'} =
          time();    # update the time field of the event
        ${$pipe}->{'BotModules_Module'}->ChildCompleted(
            ${$pipe}->{'BotModules_Event'},
            ${$pipe}->{'BotModules_ChildType'},
            $data, @{ ${$pipe}->{'BotModules_Data'} }
        );
    };
    if ($@) {
...
sub bot_select_data_available {
    my ($handle) = @_;
    &debug(
        "Module ${$handle}->{'BotModules_Module'}->{'_name'} received some data"
    );
    ...
    my $data  = '';
    my $close = 0;
    while ( select( $ready = $fh, undef, undef, 0.1 )
        and vec( $ready, fileno($handle), 1 )
        and $count < 1024
        and not $close )
    {                 # read up to 1kb
    ...
    if ( not ${$handle}->{'BotModules_Module'}->{'_shutdown'} ) {
        eval {
            ...
            ${$handle}->{'BotModules_Module'}
              ->DataAvailable( ${$handle}->{'BotModules_Event'},
                $handle, $data, $close, );
        };
        if ($@) {
...
sub killed {
    ...
    &debug(
'WARNING: SHUTTING ME DOWN LIKE THIS CAN CAUSE FORKED PROCESSES TO START UP AS BOTS!!!'
    );          # XXX which we should fix, of course.
...
sub configStructure {
    ...
    foreach (@variables) {
        confess(
"Function configStructure was passed something that is either not a ref or has not yet neem registered, so aborted"
        ) unless defined( $configStructure{$_} );
...
sub getModule {
    my ($name) = @_;
    foreach my $module (@modules)
    {   # XXX this is not cached as a hash as performance is not a priority here
...
sub LoadModule {
    ...
    foreach (@modules) {
        if ( $_->{'_name'} eq $name ) {
            return
"Failed [0]: Module already loaded. Don't forget to enable it in the various channels (vars $name channels '+#channelname').";
        }
    }
...
sub do {
    ...
    if ( $type eq 'Told' ) {
        ...
        return $self->ModeChange( $e, $e->{'channel'}, $e->{'data'},
            $e->{'from'} );
    }
...
sub schedule {
    ...
    $times = -1
      if ( $times < 0 );    # pass a negative number to have a recurring timer
    my $delay = $time;
    if ( ref($time) ) {
...
# }
    $event->{'bot'}
      ->schedule( $delay, \&doScheduled, $self, $event, $time, $times, @data );
}
        ...
        if ( defined($child) ) {
            if ($child) {
                ...
                eval {
                    ...
                    $pipe->writer()
                      ;    # get writing end of pipe, ready to output the result
                    my $output;
                    if ( ref($command) eq 'CODE' ) {
                        ...
                        my $result = IO::SecurePipe->new()
                          ;    # create a new pipe for $command
                         # call $command (implicit fork(), which may of course fail)
                        $result->reader( $command, @$arguments );
                        local $/
                          ; # to not affect the rest of the program (what little there is)
                        $/ = \( 2 * 1024 * 1024 );   # slurp up to two megabytes
                        $output =
                          <$result>;   # blocks until child process has finished
                        close($result);    # reap child
                    }
                    print $pipe $output
                      if ($output);    # output the lot in one go back to parent
                    $pipe->close();
                };
                if ($@) {
               ...
               #       <dawn> get a real os, man
                ...
                eval {
                    $0 =~
                      m/^(.*)$/os;    # untaint $0 so that we can call it below:
...
sub tellAdmin {
    ...
    if ($lastadmin) {
        ...
        $self->debug(
            "Wanted to tell an admin '$data', but I've never seen one.");
    }
}
...
sub ctcpSend {
    ...
    &::sendmsg( $event->{'bot'}, $event->{'target'}, [ $type, $data ],
        'ctcpSend' );
}
...
sub ctcpReply {
    ...
    if ( defined($data) ) {
        &::sendmsg( $event->{'bot'}, $event->{'from'}, "$type $data",
            'ctcpReply' );
    }
    else {
...
sub channelSay {
    my $self = shift;
    my ( $event, $data ) = @_;
    &::sendmsg( $event->{'bot'}, $event->{'channel'}, $data )
      if $event->{'channel'};
}
...
sub prettyPrint {
    ...
    while (@input) {
        ...
        while (
            ( $index <= $#input )
            and ( ( length( $output[$#output] ) + length( $input[$#input] ) ) <
                $preferredLineLength )
          )
        {

            # does this one fit?
            if ( ( length( $output[$#output] ) + length( $input[$index] ) ) <
                $preferredLineLength )
            {
                if ( defined($prefix) ) {
...
sub wordWrap {
    ...
    while (@input) {
        ...
        while (
            ( $#input >= 0 )
            and ( ( length( $output[$#output] ) + length( $input[0] ) ) <
                $preferredLineLength )
          )
        {
...
sub JoinedChannel {
    ...
    if ( $self->{'autojoin'} ) {
        push( @{ $self->{'channels'} }, $channel )
          unless ( ( scalar( grep $_ eq $channel, @{ $self->{'channels'} } ) )
            or
...
sub IsBanned {
    ...
    return 0
      if scalar(
              grep {
                  $_ = $self->sanitizeRegexp($_);
                  $event->{'user'} =~ /^$_$/
                } @{ $self->{'allowusers'} }
      );
    return
      scalar(
        grep { $_ = $self->sanitizeRegexp($_); $event->{'user'} =~ /^$_$/ }
          @{ $self->{'denyusers'} } );
}
...
sub RegisterConfig {
    ...
    $self->registerVariables(
        ...
        [ 'channels',        1, 1, [] ],
        [ 'channelsBlocked', 1, 1, [] ]
        ,    # the channels in which this module will not autojoin regardless
        [ 'autojoin', 1, 1, 1 ],
        [ 'allowusers', 1, 1, [] ],
        [ 'denyusers',  1, 1, [] ],
    );
...
sub Set {
    ...
    if ( $self->{'_variables'}->{$variable} ) {
        if (   ( not defined( $self->{$variable} ) )
            or ( not ref( $self->{$variable} ) ) )
        {
            $self->{$variable} = $value;
        }
            ...
            return
              1
              ; # please to not be trying to set coderefs or arrayrefs or hashrefs or ...
        }
    }
    else {
...
sub RegisterConfig {
    ...
    $self->registerVariables(
        ...
        # [ name, save?, settable?, value ]
        [ 'allowInviting', 1, 1, 1 ]
        ,    # by default, anyone can invite a bot into their channel
        [ 'allowChannelAdmin', 1, 1, 0 ]
        ,    # by default, one cannot admin from a channel
        [ 'sourceCodeCheckDelay', 1, 1, 20 ]
        ,    # by default, wait 20 seconds between source code checks
        [
            'files', 1, 1,
            [
                $0,                     'lib/Mails.pm',
                'lib/Configuration.pm', 'lib/IO/SecurePipe.pm'
            ]
        ],    # files to check for source code changes
        [ 'channels', 0, 0, undef ]
        ,     # remove the 'channels' internal variable...
        [ 'autojoin', 0, 0, 0 ],    # remove the 'autojoin' internal variable...
        [ 'errorMessagesMaxLines', 1, 1, 5 ]
        , # by default, only have 5 lines in error messages, trim middle if more
    );

    # now add in all the global variables...
    foreach ( keys %configStructure ) {
        $self->registerVariables(
            [ $configStructure{$_}[0], 0, 1, $configStructure{$_}[1] ] )
          if ( ref( $configStructure{$_}[1] ) =~ /^(?:SCALAR|ARRAY|HASH)$/go );
    }
}
    ...
    if ( $variable eq 'currentnick' ) {
        ...
        if ( $value =~ /^([-+])(.*)$/so ) {
            if ( $1 eq '+' ) {
                ...
                if ( $value eq '' ) {
                    $self->say( $event,
                        "The empty string is not a valid nick." );
                    return -1;
                }
                my $thenick = 0;
                $thenick++
                  while ( ( $thenick < @nicks )
                    and ( $value ne $nicks[$thenick] ) );
                if ( $thenick < @nicks ) {
                    $self->say( $event,
"That nick (value) is already on the list of possible nicks."
                    );
                    return -1;
                }
            }
                ...
                if ( $2 eq $nicks[$nick] ) {
                    $self->say( $event,
"You cannot remove the current nick ('$nicks[$nick]') from the list of allowed nicks... Change the 'currentnick' variable first!"
                    );
                    return -1;
                }
            }
...
sub Schedule {
    ...
    my ($event) = @_;
    $self->schedule(
        $event, \$self->{'sourceCodeCheckDelay'},
        -1, { 'action' => 'source' }
    );
...
sub Help {
    ...
    my $result  = {
        'auth' =>
'Authenticate yourself. Append the word \'quiet\' after your password if you don\'t want confirmation. Syntax: auth <username> <password> [quiet]',
        'password' =>
'Change your password: password <oldpassword> <newpassword> <newpassword>',
        'newuser' =>
'Registers a new username and password (with no privileges). Syntax: newuser <username> <newpassword> <newpassword>',
    };
...
'The administration module is used to perform tasks that fundamentally affect the bot.';
        ...
        $result->{'shutup'} =
'Clears the output queue (you actually have to say \'shutup please\' or nothing will happen).';
        $result->{'restart'} =
'Shuts the bot down completely then restarts it, so that any source changes take effect.';
        $result->{'cycle'} =
          'Makes the bot disconnect from the server then try to reconnect.';
        $result->{'changepassword'} =
'Change a user\'s password: changepassword <user> <newpassword> <newpassword>',
          $result->{'vars'} =
'Manage variables: vars [<module> [<variable> [\'<value>\']]], say \'vars\' for more details.';
        $result->{'join'} =
'Makes the bot attempt to join a channel. The same effect can be achieved using /invite. Syntax: join <channel>';
        $result->{'part'} =
'Makes the bot leave a channel. The same effect can be achieved using /kick. Syntax: part <channel>';
        $result->{'load'} =
'Loads a module from disk, if it is not already loaded: load <module>';
        $result->{'unload'} = 'Unloads a module from memory: load <module>';
        $result->{'reload'} =
          'Unloads and then loads a module: reload <module>';
        $result->{'bless'} =
          'Sets the \'admin\' flag on a registered user. Syntax: bless <user>';
        $result->{'unbless'} =
'Resets the \'admin\' flag on a registered user. Syntax: unbless <user>';
        $result->{'deleteuser'} =
          'Deletes a user from the bot. Syntax: deleteuser <username>',
          ;
    }
    return $result;
}
...
sub Told {
    ...
    return $self->SUPER::Told(@_)
      unless $self->{allowChannelAdmin}
          or $event->{channel} eq '';
    if ( $message =~
        /^\s*auth\s+($variablepattern)\s+($variablepattern)(\s+quiet)?\s*$/osi )
    {
        if ( not $event->{'channel'} ) {
            if ( defined( $users{$1} ) ) {
                if ( &::checkPassword( $2, $users{$1} ) ) {
                    ...
                    &::do( $event->{'bot'}, $event->{'_event'}, 'Authed' )
                      ;    # hack hack hack
                }
                else {
...
"You have not been added as a user yet. Try the \'newuser\' command (see \'help newuser\' for details)."
                );
            }
        }
    }
    elsif ( $message =~
/^\s*password\s+($variablepattern)\s+($variablepattern)\s+($variablepattern)\s*$/osi
      )
    {
        if ( not $event->{'channel'} ) {
            if ( $authenticatedUsers{ $event->{'user'} } ) {
                if ( $2 ne $3 ) {
                    $self->say( $event,
                        'New passwords did not match. Try again.' );
                }
                elsif (
                    &::checkPassword(
                        $1, $users{ $authenticatedUsers{ $event->{'user'} } }
                    )
                  )
                {
                    $users{ $authenticatedUsers{ $event->{'user'} } } =
                      &::newPassword($2);
                    delete( $authenticatedUsers{ $event->{'user'} } );
                    $self->say( $event,
                        'Password changed. Please reauthenticate.' );
                    $self->saveConfig();
                }
                else {
                    delete( $authenticatedUsers{ $event->{'user'} } );
...
/^\s*new\s*user\s+($variablepattern)\s+($variablepattern)\s+($variablepattern)\s*$/osi
      )
    {
        if ( not $event->{'channel'} ) {
            if ( defined( $users{$1} ) ) {
                $self->say( $event,
'That user already exists in my list, you can\'t add them again!'
                );
            }
            elsif ( $2 ne $3 ) {
                ...
                $userFlags{$1} = 0;
                $self->directSay( $event,
                    "New user '$1' added with password '$2' and no rights." );
                $self->saveConfig();
            }
            else {
        ...
        if ( $message =~ /^\s*(?:shutdown,?\s+please)\s*[?!.]*\s*$/osi ) {
            ...
            my $reason =
              'I was told to shutdown by ' . $event->{'from'} . '. :-(';
...
# XXX we don't unload the modules here?
            ...
            $self->say( $event,
                'If you really want me to shutdown, use the magic word.' );
            $self->schedule( $event, 7, 1, 'i.e., please.' );
        }
        elsif ( $message =~ /^\s*(?:restart,?\s+please)\s*[?!.]*\s*$/osi ) {
            $self->Restart( $event,
                "I was told to restart by $event->{'from'} -- brb" );
        }
        elsif ( $message =~ /^\s*restart/osi ) {
            $self->say( $event,
                'If you really want me to restart, use the magic word.' );
            $self->schedule( $event, 7, 1, 'i.e., please.' );
        }
        elsif ( $message =~ /^\s*delete\s*user\s+($variablepattern)\s*$/osi ) {
            if ( not defined( $users{$1} ) ) {
                $self->say( $event,
                    "I don't know of a user called '$1', sorry." );
            }
            else {
                ...
                if ( ( $userFlags{$doomedUser} & 1 ) == 1 ) {
                    ...
                    foreach my $user ( keys %users ) {
                        ++$count
                          if ( $user ne $doomedUser
                            and ( $userFlags{$user} & 1 ) == 1 );
                    }
                }
                else {
               ...
               # the user doing the deleting. So we're safe.
                ...
                if ($count) {
                    ...
                    $self->say( $event,
"Can't delete user '$doomedUser', that would leave you with no admins!"
                    );
                }
            }
        }
        elsif ( $message =~
/^\s*change\s*password\s+($variablepattern)\s+($variablepattern)\s+($variablepattern)\s*$/osi
          )
        {
            if ( not defined( $users{$1} ) ) {
                $self->say( $event,
                ...
                if ($count) {
                    $self->say( $event,
"Password changed for user '$1'. They must reauthenticate."
                    );
                }
                else {
            ...
            my $reason =
              'I was told to cycle by ' . $event->{'from'} . '. BRB!';
...
# XXX we don't unload the modules here?
            ...
            /^\s*join\s+([&#+][^\s]+)(?:\s+please)?\s*[?!.]*\s*$/osi )
        {
            $self->Invited( $event, $1 );
        }
        elsif ( $message =~
            /^\s*part\s+([&#+][^\s]+)(?:\s+please)?\s*[?!.]*\s*$/osi )
        {
            $event->{'bot'}
              ->part("$1 :I was told to leave by $event->{'from'}. :-(");
        }
            ...
            if ( defined( $users{$2} ) ) {
                ...
                $self->say( $event,
'I don\'t know that user. Try the \'newuser\' command (see \'help newuser\' for details).'
                );
            }
        }
            ...
            if ( defined( $users{$2} ) ) {
                ...
                $self->say( $event,
                    'I don\'t know that user. Check your spelling!' );
            }
        }
...
/^\s*vars(?:\s+($variablepattern)(?:\s+($variablepattern)(?:\s+'(.*)')?)?|(.*))?\s*$/osi
          )
        {
...
sub Authed {
    ...
    if ( $self->isAdmin($event) ) {
        foreach ( keys %userFlags ) {
            if (    ( ( $userFlags{$_} & 2 ) == 2 )
                and ( $authenticatedUsers{ $event->{'user'} } ne $_ ) )
            {
                $self->deleteUser($_);
                $self->directSay( $event,
                    "Temporary administrator '$_' removed from user list." );
            }
        }
    }
    return $self->SUPER::Authed(@_)
      ;    # this should not stop anything else happening
}
    ...
    foreach my $file ( @{ $self->{'files'} } ) {
        ...
        if ( defined($lastModifiedTime) and ( $mtime > $lastModifiedTime ) ) {
            ...
            if (1) {    # XXX replace 1 with "did compile succeed" test
                $self->Restart( $event,
'someone seems to have changed my source code. brb, unless I get a compile error!'
                );
            }
            else {
...
# XXX we don't unload modules here?
    ...
    $self->debug(
"That failed!!! Bailing out to prevent all hell from breaking loose! $@ :-|"
    );
            ...
            if ( defined($variable) ) {
                if ( defined($value) ) {
                    ...
                    if ( ( not defined($result) ) or ( $result == 0 ) ) {
                        $self->say( $event,
"Variable '$variable' in module '$modulename' has changed."
                        );
                    }
                    elsif ( $result == 1 ) {
                        $self->say( $event,
                                "Variable '$variable' is of type "
                              . ref( $module->{$variable} )
                              . ' and I do not know how to set that kind of variable!'
                        );
                    }
                    elsif ( $result == 2 ) {    # we don't know that variable!
                        if ( $module->{$variable} )
                        {                       # well we do, but only to read
                            $self->say( $event,
"Variable '$variable' in module '$modulename' is read-only, sorry."
                            );
                        }
                        else {                  # not known
                            $self->say( $event,
"Module '$modulename' does not have a variable '$variable' as far as I can tell."
                        ...
                        $self->say( $event,
"Variable '$variable' is a list. To add to a list, please use the '+' symbol before the value (vars <module> <variable> '+<value>'). To remove from a list, use the '-' symbol (vars <module> <variable> '-<value>')."
                        );
                    }
                    elsif ( $result == 4 ) {
                        $self->say( $event,
"Variable '$variable' is a hash. To add to a hash, please use the '+' symbol before the '|key|value' pair (vars <module> <variable> '+|<key>|<value>').  The separator symbol ('|' in this example) could be anything. To remove from a list, use the '-' symbol (vars <module> <variable> '-<key>')."
                        );
                    }
                    elsif ( $result == -1 ) {
                        ...
                        $self->say( $event,
"Variable '$variable' in module '$modulename' has changed, but may not be what you expect since it appears to me that you used a letter to delimit the sections. I hope that is what you meant to do..."
                        );
                    }
                    elsif ( $result > 0 ) {    # negative = success
                        $self->say( $event,
"Variable '$variable' in module '$modulename' could not be set for some reason unknown to me."
                        );
                    }
                }
                    ...
                    if ( defined($value) ) {
                        ...
                        if ( $type eq 'SCALAR' ) {
                            $self->say( $event,
"Variable '$variable' in module '$modulename' is set to: '$$value'"
                            );
                        }
                        elsif ( $type eq 'ARRAY' ) {
...
# XXX need a 'maximum number of items' feature to prevent flooding ourselves to pieces (or is shutup please enough?)
                            if (@$value) {
                                local $" = '\', \'';
                                $self->say( $event,
"Variable '$variable' in module '$modulename' is a list with the following values: '@$value'"
                                );
                            }
                            else {
                                $self->say( $event,
"Variable '$variable' in module '$modulename' is an empty list."
                                );
                            }
                        }
                        elsif ( $type eq 'HASH' ) {
...
# XXX need a 'maximum number of items' feature to prevent flooding ourselves to pieces (or is shutup please enough?)
                            $self->say( $event,
"Variable '$variable' in module '$modulename' is a hash with the following values:"
                            );
                            ...
                            $self->say( $event,
                                "End of dump of variable '$variable'." );
                        }
                        else {
                            $self->say( $event,
"Variable '$variable' in module '$modulename' is set to: '$value'"
                        ...
                        if ( $module->{'_variables'}->{$variable} )
                        {     # well we do, but only to write
                            $self->say( $event,
...
"Module '$modulename' does not have a variable '$variable' as far as I can tell."
                ...
                if (@variables) {
                    local $" = '\', \'';
                    $self->say( $event,
"Module '$modulename' has the following published variables: '@variables'"
                    );
                }
                else {
                    $self->say( $event,
                        "Module '$modulename' has no settable variables." );
                }
            }
        }
        else {    # complain no module
            $self->say( $event,
"I didn't recognise that module name ('$modulename'). Try just 'vars' on its own for help."
            );
        }
    }
    elsif ($nonsense) {
...
sub Invited {
    ...
    if ( grep $_ eq $channel, @channels ) {
        $self->directSay( $event,
            "I thought I was already *in* channel $channel! Oh well." );
    }
    if ( $self->isAdmin($event) || $self->{'allowInviting'} ) {
        ...
        $self->debug(
            $event->{'from'} . " asked me to join $channel, but I refused." );
        $self->directSay( $event,
"Please contact one of my administrators if you want me to join $channel."
        );
        $self->tellAdmin( $event,
                "Excuse me, but "
              . $event->{'from'}
              . " asked me to join $channel. I thought you should know." );
    }
...
sub LoadModule {
    ...
    if ( ref($newmodule) ) {
        ...
        eval { $newmodule->Schedule($event); };
        if ($@) {
            $self->debug(
                "Warning: An error occured while loading the module:\n$@");
            if ($requested) {
                $self->say( $event,
"Warning: an error occured while loading module '$name'. Ignored."
                );
            }
        }
        ...
        if ($requested) {    # it failed, $newmodule contains error message
            ...
            if ( scalar(@errors) > $self->{'errorMessagesMaxLines'} ) {
                ...
                @errors = (
                    @errors[ 0 .. int( $self->{'errorMessagesMaxLines'} / 2 ) -
                      1 ],
                    '...',
                    @errors[
                      -( int( $self->{'errorMessagesMaxLines'} / 2 ) ) .. -1
                    ]
                );
            }
            local $" = "\n";
